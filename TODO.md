...
- [x] **s2_latlng_rect_bounder** - `S2_latlng_rect_bounder`
...
- [x] **`s2_cell_union.union`: merge-aware normalize**
...
- [x] **`s2_loop_measures.surface_integral_r3`: unbox the R3 accumulator**
...
- [x] **`s2_padded_cell`** - annotated; `create`, `child_ij`, `shrink_to_fit`,
  and `sexp_of_t` marked ignore. Pure accessors rely on module-level
  annotation.
...
- [x] **`s2_region_coverer`** - annotated; `create`, `covering`,
  `interior_covering`, `fast_covering`, and `canonicalize_covering` marked
  ignore. `is_canonical` and pure parameter accessors rely on module-level
  annotation.
- [x] **`s2_cell_union`** - annotated; constructors, bulk operations, set
  operations, bounds, and `sexp_of_t` marked ignore. Predicates and
  per-cell accessors rely on module-level annotation.
