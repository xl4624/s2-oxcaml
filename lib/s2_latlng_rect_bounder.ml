...
(* [a] / [a_latlng] are sentinels until the first [add_internal] call. They are only
   inspected once [bound] is non-empty, so the initial values never leak into the result. *)
let[@inline] [@zero_alloc] create () =
  #{ a = S2_point.origin; a_latlng = S2_latlng.zero; bound = S2_latlng_rect.empty }
;;

(* Core update. Accumulates the geodesic edge (a, b) into [bound]. The interesting work is
   bounding the latitude range: a great circle can reach above or below both endpoints, so
   we check whether the edge's maximum latitude lies in its interior and adjust
   accordingly. Magic numbers like 1.91346e-15 and 6.06638e-16 come from the error
   analysis in the upstream source; see s2latlng_rect_bounder.cc:AddInternal. *)
let add_internal (t : t) (b : S2_point.t) (b_latlng : S2_latlng.t) : t =
  ...
  then #{ a = b; a_latlng = b_latlng; bound = S2_latlng_rect.add_point t.#bound b_latlng }
    ...
    (* N = (a - b) x (a + b) = 2 * (a x b); computing it this way retains precision when
       [a] and [b] are close to each other, where a direct cross product would subtract
    ...
    let new_bound =
      ...
      then
        ...
        (* [m] is perpendicular to the plane through [n] and the Z axis. The sign of
           dot(m, a) vs dot(m, b) tells us whether the latitude extremum lies in the
           interior of the edge (signs differ) or at an endpoint (signs agree). *)
        ...
        let lat_ab =
          ...
          then (
            (* The latitude extremum may lie in the interior of the edge. [max_lat] is the
               theoretical maximum latitude on the great circle; the "latitude budget"
               then bounds how much farther the actual edge can reach beyond its
               endpoints. *)
...
let[@zero_alloc] get_bound t =
  (* Pad latitude by 2*epsilon to absorb the worst-case rounding when a caller re-projects
     an S2Point back to S2LatLng, then take polar_closure so that a bound that touches a
     pole covers every longitude at that pole. *)
  let open Float_u.O in
