...
let contains_sign t =
  (* Find the unmatched edge that is immediately clockwise from ref_dir(target)
     but not equal to it. The result is +1 iff this edge is outgoing.

     A loop with consecutive vertices A, B, C contains vertex B iff the fixed
     vector R = ref_dir(B) is contained by the wedge ABC, which is closed at A
     and open at C. This convention matches [S2_edge_crossings.vertex_crossing]. *)
