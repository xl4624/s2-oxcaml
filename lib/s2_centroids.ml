...
let[@inline] [@zero_alloc] planar_centroid a b c =
  ...
  R3_vector.mul sum (Float_u.of_float (1.0 /. 3.0))
;;

let[@zero_alloc] true_centroid a b c =
  let open Float_u.O in
  (* Use angle (distance) to get accurate results for small triangles. *)
  ...
  (* Solve using Cramer's rule with row subtraction for numerical stability. *)
