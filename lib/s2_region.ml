...
(* True iff the cap interior reaches some point of the cell EXCLUDING the already-checked
   vertices. Used by [cap_contains_cell] and [cap_intersects_cell] to cover the cases
   where an edge of the cell crosses the cap boundary between vertex samples.

   Method: for each cell edge, compute the closest approach of the cap center to the great
   circle supporting the edge. If the perpendicular distance exceeds the cap radius the
   edge misses entirely. Otherwise project the closest point back onto the edge segment
   and report a hit if it lies between the two endpoints. A local recursive helper
   replaces a [for] loop so we can return as soon as the answer is decided - in coverer
   hot paths most edges reject on the first test. *)
