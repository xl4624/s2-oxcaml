(* C++ test parity: s2geometry/src/s2/s2cap_test.cc
   Golden data from test/gen/s2cap.cc.

   Covered:
   -  TEST(S2Cap, Basic)
   -  TEST(S2Cap, AddEmptyCapToNonEmptyCap) + AddNonEmptyCapToEmptyCap (via add_cap)
   -  TEST(S2Cap, GetRectBound)
   -  TEST(S2Cap, Expanded)
   -  TEST(S2Cap, GetCentroid)
   -  TEST(S2Cap, Union)
   -  TEST(S2Cap, EncodeDecode)

   Deliberately omitted:
   -  TEST(S2Cap, S2CellMethods) — requires S2_cell
   -  TEST(S2Cap, GetCellUnionBoundLevel1Radius) — requires S2_metrics / cell cover
   -  TEST(S2Cap, S2CoderWorks) — legacy coder not ported *)

open Core
open Test_helpers

let k_degree_eps = 1e-13
(* -- Quickcheck generators ------------------------------------------------ *)

module Cap_gen = struct
  type t = { cap : S2.S2_cap.t } [@@deriving sexp_of]

  let quickcheck_generator =
    let open Base_quickcheck.Generator in
    let coord = float_inclusive (-1.0) 1.0 in
    let radius = float_inclusive (-0.1) 4.0 in
    (* Includes empty (<0) and full (>pi) *)
    create (fun ~size:_ ~random:rnd ->
      let x = generate coord ~size:30 ~random:rnd in
      let y = generate coord ~size:30 ~random:rnd in
      let z = generate coord ~size:30 ~random:rnd in
      let r = generate radius ~size:30 ~random:rnd in
      let center =
        S2.S2_point.of_coords
          ~x:(Float_u.of_float x)
          ~y:(Float_u.of_float y)
          ~z:(Float_u.of_float z)
      in
      let chord_angle = S2.S1_chord_angle.of_length2 r in
      { cap = S2.S2_cap.of_center_chord_angle center chord_angle })
  ;;

  let quickcheck_shrinker = Base_quickcheck.Shrinker.atomic
end

module Cap_pair = struct
  type t =
    { a : S2.S2_cap.t
    ; b : S2.S2_cap.t
    }
  [@@deriving sexp_of]

  let quickcheck_generator =
    let open Base_quickcheck.Generator in
    let coord = float_inclusive (-1.0) 1.0 in
    let radius = float_inclusive (-0.1) 4.0 in
    let gen_cap rnd =
      let x = generate coord ~size:30 ~random:rnd in
      let y = generate coord ~size:30 ~random:rnd in
      let z = generate coord ~size:30 ~random:rnd in
      let r = generate radius ~size:30 ~random:rnd in
      let center =
        S2.S2_point.of_coords
          ~x:(Float_u.of_float x)
          ~y:(Float_u.of_float y)
          ~z:(Float_u.of_float z)
      in
      let chord_angle = S2.S1_chord_angle.of_length2 r in
      S2.S2_cap.of_center_chord_angle center chord_angle
    in
    create (fun ~size:_ ~random:rnd -> { a = gen_cap rnd; b = gen_cap rnd })
  ;;

  let quickcheck_shrinker = Base_quickcheck.Shrinker.atomic
end

let qc_config =
  let module T = Base_quickcheck.Test in
  { T.default_config with test_count = 400; shrink_count = 100 }
;;

let quickcheck_contains_center () =
  Base_quickcheck.Test.run_exn
    (module Cap_gen)
    ~config:qc_config
    ~f:(fun { Cap_gen.cap } ->
      if not (S2.S2_cap.is_empty cap)
      then assert (S2.S2_cap.contains_point cap (S2.S2_cap.center cap)))
;;

let quickcheck_complement_complement () =
  Base_quickcheck.Test.run_exn
    (module Cap_gen)
    ~config:qc_config
    ~f:(fun { Cap_gen.cap } ->
      let c = S2.S2_cap.complement cap in
      let cc = S2.S2_cap.complement c in
      assert (S2.S2_cap.approx_equal ~max_error:1e-14 cap cc))
;;

(*TODO: verify this test *)
let quickcheck_union_contains_both () =
  Base_quickcheck.Test.run_exn
    (module Cap_pair)
    ~config:qc_config
    ~f:(fun { Cap_pair.a; b } ->
      let u = S2.S2_cap.union a b in
      let margin = S2.S1_angle.of_radians (Float_u.of_float 1e-14) in
      let u_expanded = S2.S2_cap.Option.value_exn (S2.S2_cap.expanded u margin) in
      assert (S2.S2_cap.contains_cap u_expanded a);
      assert (S2.S2_cap.contains_cap u_expanded b))
;;

let quickcheck_add_point_contains () =
  Base_quickcheck.Test.run_exn
    (module Cap_gen)
    ~config:qc_config
    ~f:(fun { Cap_gen.cap } ->
      let p =
        S2.S2_point.of_coords
          ~x:(Float_u.of_float 1.0)
          ~y:(Float_u.of_float 0.0)
          ~z:(Float_u.of_float 0.0)
      in
      let added = S2.S2_cap.add_point cap p in
      assert (S2.S2_cap.contains_point added p))
;;

let quickcheck_empty_subset_any () =
  Base_quickcheck.Test.run_exn
    (module Cap_gen)
    ~config:qc_config
    ~f:(fun { Cap_gen.cap } -> assert (S2.S2_cap.contains_cap cap S2.S2_cap.empty))
;;

let quickcheck_any_subset_full () =
  Base_quickcheck.Test.run_exn
    (module Cap_gen)
    ~config:qc_config
    ~f:(fun { Cap_gen.cap } -> assert (S2.S2_cap.contains_cap S2.S2_cap.full cap))
;;

(* -- End quickcheck ------------------------------------------------------- *)

let latlng_point lat_deg lng_deg =
  S2.S2_latlng.to_point
    (S2.S2_latlng.of_degrees
       ~lat:(Float_u.of_float lat_deg)
       ~lng:(Float_u.of_float lng_deg))
;;

let cap_of_json j =
  let center = r3_vector_of_json (member "center" j) in
  let length2 = float_of_json_exn (member "length2" j) in
  S2.S2_cap.of_center_chord_angle center (S2.S1_chord_angle.of_length2 length2)
;;

let check_lat_lng_rect msg (expected_r : Yojson.Safe.t) (actual : S2.S2_cap.lat_lng_rect) =
  let lat_j = member "lat" expected_r in
  let lng_j = member "lng" expected_r in
  let exp_lat = r1_interval_of_json lat_j in
  let exp_lng = s1_interval_of_json lng_j in
  check_float
    (msg ^ " lat lo")
    ~expected:(Float_u.to_float (S2.R1_interval.lo exp_lat))
    ~actual:(Float_u.to_float (S2.R1_interval.lo actual.lat));
  check_float
    (msg ^ " lat hi")
    ~expected:(Float_u.to_float (S2.R1_interval.hi exp_lat))
    ~actual:(Float_u.to_float (S2.R1_interval.hi actual.lat));
  check_float
    (msg ^ " lng lo")
    ~expected:(Float_u.to_float (S2.S1_interval.lo exp_lng))
    ~actual:(Float_u.to_float (S2.S1_interval.lo actual.lng));
  check_float
    (msg ^ " lng hi")
    ~expected:(Float_u.to_float (S2.S1_interval.hi exp_lng))
    ~actual:(Float_u.to_float (S2.S1_interval.hi actual.lng));
  check_bool
    (msg ^ " lat_empty")
    ~expected:(bool_of_json_exn (member "lat_empty" expected_r))
    ~actual:(S2.R1_interval.is_empty actual.lat);
  check_bool
    (msg ^ " lng_full")
    ~expected:(bool_of_json_exn (member "lng_full" expected_r))
    ~actual:(S2.S1_interval.is_full actual.lng)
;;

let test_basic f () =
  let b = member "basic" f in
  let chk name pred =
    Alcotest.(check bool) name (bool_of_json_exn (member name b)) pred
  in
  let chk_f name pred =
    check_float name ~expected:(float_of_json_exn (member name b)) ~actual:pred
  in
  chk "empty_valid" (S2.S2_cap.is_valid S2.S2_cap.empty);
  chk "empty_is_empty" (S2.S2_cap.is_empty S2.S2_cap.empty);
  chk "empty_complement_full" (S2.S2_cap.is_full (S2.S2_cap.complement S2.S2_cap.empty));
  chk "full_valid" (S2.S2_cap.is_valid S2.S2_cap.full);
  chk "full_is_full" (S2.S2_cap.is_full S2.S2_cap.full);
  chk "full_complement_empty" (S2.S2_cap.is_empty (S2.S2_cap.complement S2.S2_cap.full));
  chk_f "full_height" (S2.S2_cap.height S2.S2_cap.full);
  chk_f
    "full_radius_deg"
    (Float_u.to_float (S2.S1_angle.degrees (S2.S2_cap.radius_angle S2.S2_cap.full)));
  chk
    "neg_angle_empty"
    (S2.S2_cap.is_empty
       (S2.S2_cap.of_center_angle
          (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)
          (S2.S1_angle.of_radians (Float_u.of_float (-20.0)))));
  chk
    "large_angle_full"
    (S2.S2_cap.is_full
       (S2.S2_cap.of_center_angle
          (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)
          (S2.S1_angle.of_radians (Float_u.of_float 5.0))));
  chk
    "infinity_angle_full"
    (S2.S2_cap.is_full
       (S2.S2_cap.of_center_angle
          (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)
          S2.S1_angle.infinity));
  let ce = S2.S2_cap.center S2.S2_cap.empty in
  let re = S2.S2_cap.radius_chord S2.S2_cap.empty in
  let default_empty = S2.S2_cap.of_center_chord_angle ce re in
  (* C++ default [S2Cap] matches [Empty()] center and height. *)
  chk
    "default_equals_empty_center"
    (S2.S2_point.equal
       (S2.S2_cap.center S2.S2_cap.empty)
       (S2.S2_cap.center default_empty));
  chk
    "default_equals_empty_height"
    (Float.( = ) (S2.S2_cap.height S2.S2_cap.empty) (S2.S2_cap.height default_empty));
  chk "empty_contains_empty" (S2.S2_cap.contains_cap S2.S2_cap.empty S2.S2_cap.empty);
  chk "full_contains_empty" (S2.S2_cap.contains_cap S2.S2_cap.full S2.S2_cap.empty);
  chk "full_contains_full" (S2.S2_cap.contains_cap S2.S2_cap.full S2.S2_cap.full);
  chk
    "empty_interior_intersects_empty"
    (S2.S2_cap.interior_intersects S2.S2_cap.empty S2.S2_cap.empty);
  chk
    "full_interior_intersects_full"
    (S2.S2_cap.interior_intersects S2.S2_cap.full S2.S2_cap.full);
  chk
    "full_interior_intersects_empty"
    (S2.S2_cap.interior_intersects S2.S2_cap.full S2.S2_cap.empty);
  let xaxis = S2.S2_cap.of_point (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0) in
  let yaxis = S2.S2_cap.of_point (S2.S2_point.of_coords ~x:#0.0 ~y:#1.0 ~z:#0.0) in
  chk
    "xaxis_contains_axis"
    (S2.S2_cap.contains_point xaxis (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0));
  (* Golden value is C++ [Contains(normalized near point)] (false for exterior). *)
  let np = r3_vector_of_json (member "xaxis_near_point" b) in
  let near =
    S2.S2_point.of_coords
      ~x:(S2.R3_vector.x np)
      ~y:(S2.R3_vector.y np)
      ~z:(S2.R3_vector.z np)
  in
  chk "xaxis_not_contains_near" (S2.S2_cap.contains_point xaxis near);
  chk_f
    "xaxis_radius_rad"
    (Float_u.to_float (S2.S1_angle.radians (S2.S2_cap.radius_angle xaxis)));
  (* Golden = C++ [yaxis.Contains(xaxis.center())]. *)
  chk
    "yaxis_not_contains_xcenter"
    (S2.S2_cap.contains_point yaxis (S2.S2_cap.center xaxis));
  chk_f "xaxis_height" (S2.S2_cap.height xaxis);
  let xcomp = S2.S2_cap.complement xaxis in
  chk "xcomp_full" (S2.S2_cap.is_full xcomp);
  chk "xcomp_contains_center" (S2.S2_cap.contains_point xcomp (S2.S2_cap.center xaxis));
  let xcc = S2.S2_cap.complement xcomp in
  chk "xcompcomp_empty" (S2.S2_cap.is_empty xcc);
  chk
    "xcompcomp_not_contains"
    (not (S2.S2_cap.contains_point xcc (S2.S2_cap.center xaxis)));
  let k_tiny = 1e-10 in
  let tiny =
    S2.S2_cap.of_center_angle
      (S2.S2_point.of_coords ~x:#1.0 ~y:#2.0 ~z:#3.0)
      (S2.S1_angle.of_radians (Float_u.of_float k_tiny))
  in
  let tc = S2.S2_cap.center tiny in
  let tangent =
    S2.R3_vector.normalize
      (S2.R3_vector.cross tc (S2.R3_vector.create ~x:#3.0 ~y:#2.0 ~z:#1.0))
  in
  let inner =
    S2.R3_vector.normalize
      (S2.R3_vector.add tc (S2.R3_vector.mul tangent (Float_u.of_float (0.99 *. k_tiny))))
  in
  let outer =
    S2.R3_vector.normalize
      (S2.R3_vector.add tc (S2.R3_vector.mul tangent (Float_u.of_float (1.01 *. k_tiny))))
  in
  chk "tiny_contains_inner" (S2.S2_cap.contains_point tiny inner);
  chk "tiny_not_contains_outer" (not (S2.S2_cap.contains_point tiny outer));
  let hemi =
    S2.S2_cap.of_center_height (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#1.0) 1.0
  in
  let hemi_c = S2.S2_cap.center hemi in
  chk
    "hemi_complement_center_neg"
    (S2.S2_point.equal
       (S2.S2_cap.center (S2.S2_cap.complement hemi))
       (S2.R3_vector.neg hemi_c));
  chk_f "hemi_complement_height" (S2.S2_cap.height (S2.S2_cap.complement hemi));
  chk
    "hemi_contains_x"
    (S2.S2_cap.contains_point hemi (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0));
  chk
    "hemi_comp_not_x"
    (not
       (S2.S2_cap.contains_point
          (S2.S2_cap.complement hemi)
          (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)));
  let k_eps = #1e-15 in
  chk
    "hemi_contains_near"
    (S2.S2_cap.contains_point
       hemi
       (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:Float_u.(Float_u.neg (#1.0 - k_eps))));
  chk
    "hemi_not_interior"
    (not
       (S2.S2_cap.interior_contains_point
          hemi
          (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:Float_u.(Float_u.neg (#1.0 + k_eps)))));
  let concave_center = latlng_point 80.0 10.0 in
  let concave =
    S2.S2_cap.of_center_chord_angle
      concave_center
      (S2.S1_chord_angle.of_angle (S2.S1_angle.of_degrees (Float_u.of_float 150.0)))
  in
  let max_err =
    S2.S1_chord_angle.max_point_error (S2.S2_cap.radius_chord concave)
    +. S2.S1_chord_angle.max_angle_error (S2.S2_cap.radius_chord concave)
    +. (3.0 *. Float.epsilon_float)
  in
  let concave_min =
    S2.S2_cap.of_center_chord_angle
      concave_center
      (S2.S1_chord_angle.plus_error (S2.S2_cap.radius_chord concave) (-.max_err))
  in
  let concave_max =
    S2.S2_cap.of_center_chord_angle
      concave_center
      (S2.S1_chord_angle.plus_error (S2.S2_cap.radius_chord concave) max_err)
  in
  chk
    "concave_max_contains_a"
    (S2.S2_cap.contains_point concave_max (latlng_point (-70.0) 10.0));
  chk
    "concave_min_not_a"
    (not (S2.S2_cap.contains_point concave_min (latlng_point (-70.0) 10.0)));
  chk
    "concave_max_contains_b"
    (S2.S2_cap.contains_point concave_max (latlng_point (-50.0) (-170.0)));
  chk
    "concave_min_not_b"
    (not (S2.S2_cap.contains_point concave_min (latlng_point (-50.0) (-170.0))));
  chk "empty_not_contains_xaxis" (not (S2.S2_cap.contains_cap S2.S2_cap.empty xaxis));
  chk
    "empty_not_interior_xaxis"
    (not (S2.S2_cap.interior_intersects S2.S2_cap.empty xaxis));
  chk "full_contains_xaxis" (S2.S2_cap.contains_cap S2.S2_cap.full xaxis);
  chk "full_interior_xaxis" (S2.S2_cap.interior_intersects S2.S2_cap.full xaxis);
  chk "xaxis_not_contains_full" (not (S2.S2_cap.contains_cap xaxis S2.S2_cap.full));
  chk "xaxis_not_interior_full" (not (S2.S2_cap.interior_intersects xaxis S2.S2_cap.full));
  chk "xaxis_contains_self" (S2.S2_cap.contains_cap xaxis xaxis);
  chk "xaxis_not_interior_self" (not (S2.S2_cap.interior_intersects xaxis xaxis));
  chk "xaxis_contains_empty" (S2.S2_cap.contains_cap xaxis S2.S2_cap.empty);
  chk
    "xaxis_not_interior_empty"
    (not (S2.S2_cap.interior_intersects xaxis S2.S2_cap.empty));
  chk "hemi_contains_tiny" (S2.S2_cap.contains_cap hemi tiny);
  let pi4 = Float_u.(Float_u.pi () / #4.0) in
  chk
    "hemi_contains_small_cap"
    (S2.S2_cap.contains_cap
       hemi
       (S2.S2_cap.of_center_angle
          (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)
          (S2.S1_angle.of_radians Float_u.(pi4 - k_eps))));
  chk
    "hemi_not_contains_larger"
    (not
       (S2.S2_cap.contains_cap
          hemi
          (S2.S2_cap.of_center_angle
             (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)
             (S2.S1_angle.of_radians Float_u.(pi4 + k_eps)))));
  chk "concave_contains_hemi" (S2.S2_cap.contains_cap concave hemi);
  chk
    "concave_interior_intersects_hemi_comp"
    (S2.S2_cap.interior_intersects concave (S2.S2_cap.complement hemi));
  chk
    "concave_not_contains_antipode_cap"
    (not
       (S2.S2_cap.contains_cap
          concave
          (S2.S2_cap.of_center_height (S2.R3_vector.neg (S2.S2_cap.center concave)) 0.1)))
;;

let test_add_cap f () =
  let a = member "add_cap" f in
  let non_empty =
    S2.S2_cap.of_center_angle
      (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 10.0))
  in
  let t1 = S2.S2_cap.add_cap non_empty S2.S2_cap.empty in
  check_float_u
    "add_empty_area"
    ~expected:(float_u_of_json_exn (member "add_empty_area" a))
    ~actual:(S2.S2_cap.area t1);
  let t2 = S2.S2_cap.add_cap S2.S2_cap.empty non_empty in
  check_float_u
    "empty_after_add_area"
    ~expected:(float_u_of_json_exn (member "empty_after_add_area" a))
    ~actual:(S2.S2_cap.area t2)
;;

let test_rect_bound f () =
  let cases = to_list (member "rect_bound" f) in
  List.iter cases ~f:(fun c ->
    let name = string_of_json_exn (member "name" c) in
    let rect_j = member "rect" c in
    let cap =
      match name with
      | "empty" -> S2.S2_cap.empty
      | "full" -> S2.S2_cap.full
      | "south_pole_cap" ->
        S2.S2_cap.of_center_angle
          (latlng_point (-45.0) 57.0)
          (S2.S1_angle.of_degrees (Float_u.of_float 50.0))
      | "north_tangent" ->
        S2.S2_cap.of_center_angle
          (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#1.0)
          (S2.S1_angle.of_radians (Float_u.of_float ((Float.pi /. 4.0) +. 1e-16)))
      | "north_45_eps" ->
        S2.S2_cap.of_center_angle
          (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#1.0)
          (S2.S1_angle.of_degrees (Float_u.of_float (45.0 +. 5e-15)))
      | "eastern_hemi" ->
        S2.S2_cap.of_center_angle
          (S2.S2_point.of_coords ~x:#0.0 ~y:#1.0 ~z:#0.0)
          (S2.S1_angle.of_radians (Float_u.of_float ((Float.pi /. 2.0) +. 2e-16)))
      | "equator_50" ->
        S2.S2_cap.of_center_angle
          (latlng_point 0.0 50.0)
          (S2.S1_angle.of_degrees (Float_u.of_float 20.0))
      | "north_pole_cap" ->
        S2.S2_cap.of_center_angle
          (latlng_point 90.0 123.0)
          (S2.S1_angle.of_degrees (Float_u.of_float 10.0))
      | _ ->
        (match Alcotest.fail ("unknown rect_bound case " ^ name) with
         | (_ : Nothing.t) -> .)
    in
    check_lat_lng_rect name rect_j (S2.S2_cap.rect_bound cap))
;;

let test_expanded f () =
  let e = member "expanded" f in
  let chk name pred =
    Alcotest.(check bool) name (bool_of_json_exn (member name e)) pred
  in
  chk
    "empty_stays_empty"
    (S2.S2_cap.is_empty
       (S2.S2_cap.Option.value_exn
          (S2.S2_cap.expanded
             S2.S2_cap.empty
             (S2.S1_angle.of_radians (Float_u.of_float 2.0)))));
  chk
    "full_stays_full"
    (S2.S2_cap.is_full
       (S2.S2_cap.Option.value_exn
          (S2.S2_cap.expanded
             S2.S2_cap.full
             (S2.S1_angle.of_radians (Float_u.of_float 2.0)))));
  let cap50 =
    S2.S2_cap.of_center_angle
      (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 50.0))
  in
  let cap51 =
    S2.S2_cap.of_center_angle
      (S2.S2_point.of_coords ~x:#1.0 ~y:#0.0 ~z:#0.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 51.0))
  in
  let exp0 =
    S2.S2_cap.Option.value_exn
      (S2.S2_cap.expanded cap50 (S2.S1_angle.of_radians (Float_u.of_float 0.0)))
  in
  Alcotest.(check bool) "expanded0_approx_cap50" true (S2.S2_cap.approx_equal exp0 cap50);
  check_float_u
    "cap50_exp0_len2"
    ~expected:(float_u_of_json_exn (member "cap50_exp0_len2" e))
    ~actual:(S2.S1_chord_angle.length2 (S2.S2_cap.radius_chord exp0));
  let exp1 =
    S2.S2_cap.Option.value_exn
      (S2.S2_cap.expanded cap50 (S2.S1_angle.of_degrees (Float_u.of_float 1.0)))
  in
  check_float_u
    "cap50_exp1_len2"
    ~expected:(float_u_of_json_exn (member "cap50_exp1_len2" e))
    ~actual:(S2.S1_chord_angle.length2 (S2.S2_cap.radius_chord exp1));
  check_float_u
    "cap51_len2"
    ~expected:(float_u_of_json_exn (member "cap51_len2" e))
    ~actual:(S2.S1_chord_angle.length2 (S2.S2_cap.radius_chord cap51));
  chk
    "exp12999_not_full"
    (not
       (S2.S2_cap.is_full
          (S2.S2_cap.Option.value_exn
             (S2.S2_cap.expanded cap50 (S2.S1_angle.of_degrees (Float_u.of_float 129.99))))));
  chk
    "exp13001_full"
    (S2.S2_cap.is_full
       (S2.S2_cap.Option.value_exn
          (S2.S2_cap.expanded cap50 (S2.S1_angle.of_degrees (Float_u.of_float 130.01)))))
;;

let test_centroid f () =
  let c = member "centroid" f in
  check_r3_vector_exact
    "empty_centroid"
    ~expected:(r3_vector_of_json (member "empty_centroid" c))
    ~actual:(S2.S2_cap.centroid S2.S2_cap.empty);
  check_float
    "full_centroid_norm"
    ~expected:(float_of_json_exn (member "full_centroid_norm" c))
    ~actual:(Float_u.to_float (S2.R3_vector.norm (S2.S2_cap.centroid S2.S2_cap.full)));
  let samples = to_list (member "samples" c) in
  List.iteri samples ~f:(fun i s ->
    let center = r3_vector_of_json (member "center" s) in
    let height = float_of_json_exn (member "height" s) in
    let cap = S2.S2_cap.of_center_height center height in
    let centroid = S2.S2_cap.centroid cap in
    let expected = r3_vector_of_json (member "expected" s) in
    check_r3_vector (sprintf "centroid sample %d" i) ~expected ~actual:centroid)
;;

let test_union f () =
  let a0 =
    S2.S2_cap.of_center_angle
      (latlng_point 50.0 10.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 0.2))
  in
  let b0 =
    S2.S2_cap.of_center_angle
      (latlng_point 50.0 10.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 0.3))
  in
  let c0 =
    S2.S2_cap.of_center_angle
      (latlng_point 51.0 11.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 1.5))
  in
  let d0 =
    S2.S2_cap.of_center_angle
      (latlng_point 51.0 11.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 0.1))
  in
  let e0 =
    S2.S2_cap.of_center_angle
      (latlng_point 50.3 10.3)
      (S2.S1_angle.of_degrees (Float_u.of_float 0.2))
  in
  let f0 =
    S2.S2_cap.of_center_angle
      (S2.S2_point.of_coords ~x:#0.0 ~y:#0.0 ~z:#1.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 150.0))
  in
  let g0 =
    S2.S2_cap.of_center_angle
      (S2.S2_point.of_coords ~x:#0.0 ~y:#1.0 ~z:#0.0)
      (S2.S1_angle.of_degrees (Float_u.of_float 150.0))
  in
  let hemi =
    S2.S2_cap.of_center_height (S2.S2_point.of_coords ~x:#0.0 ~y:#0.0 ~z:#1.0) 1.0
  in
  let cases = to_list (member "union" f) in
  List.iter cases ~f:(fun c ->
    let name = string_of_json_exn (member "name" c) in
    match name with
    | "same_center_larger" ->
      let u = S2.S2_cap.union a0 b0 in
      check_bool
        "b_contains_a"
        ~expected:(bool_of_json_exn (member "b_contains_a" c))
        ~actual:(S2.S2_cap.contains_cap b0 a0);
      check_bool
        "eq"
        ~expected:(bool_of_json_exn (member "eq" c))
        ~actual:(S2.S2_cap.equal u b0)
    | "with_full" ->
      check_bool
        "is_full"
        ~expected:(bool_of_json_exn (member "is_full" c))
        ~actual:(S2.S2_cap.is_full (S2.S2_cap.union a0 S2.S2_cap.full))
    | "with_empty" ->
      check_bool
        "area_eq"
        ~expected:(bool_of_json_exn (member "area_eq" c))
        ~actual:
          (Float_u.equal
             (S2.S2_cap.area (S2.S2_cap.union a0 S2.S2_cap.empty))
             (S2.S2_cap.area a0))
    | "encompasses" ->
      let u = S2.S2_cap.union a0 c0 in
      check_bool
        "contains"
        ~expected:(bool_of_json_exn (member "contains" c))
        ~actual:(S2.S2_cap.contains_cap c0 a0);
      check_bool
        "center_eq"
        ~expected:(bool_of_json_exn (member "center_eq" c))
        ~actual:(S2.S2_point.equal (S2.S2_cap.center u) (S2.S2_cap.center c0));
      check_bool
        "radius_eq"
        ~expected:(bool_of_json_exn (member "radius_eq" c))
        ~actual:(S2.S1_angle.equal (S2.S2_cap.radius_angle u) (S2.S2_cap.radius_angle c0))
    | "disjoint" ->
      let u = S2.S2_cap.union a0 d0 in
      let ll = S2.S2_latlng.of_point (S2.S2_cap.center u) in
      check_bool
        "not_contains"
        ~expected:(bool_of_json_exn (member "not_contains" c))
        ~actual:(not (S2.S2_cap.contains_cap d0 a0));
      check_bool
        "not_intersects"
        ~expected:(bool_of_json_exn (member "not_intersects" c))
        ~actual:(not (S2.S2_cap.intersects d0 a0));
      check_float
        ~eps:k_degree_eps
        "lat_deg"
        ~expected:(float_of_json_exn (member "lat_deg" c))
        ~actual:(Float_u.to_float (S2.S1_angle.degrees (S2.S2_latlng.lat ll)));
      check_float
        ~eps:k_degree_eps
        "lng_deg"
        ~expected:(float_of_json_exn (member "lng_deg" c))
        ~actual:(Float_u.to_float (S2.S1_angle.degrees (S2.S2_latlng.lng ll)));
      check_float
        ~eps:k_degree_eps
        "radius_deg"
        ~expected:(float_of_json_exn (member "radius_deg" c))
        ~actual:(Float_u.to_float (S2.S1_angle.degrees (S2.S2_cap.radius_angle u)))
    | "overlap" ->
      let u = S2.S2_cap.union a0 e0 in
      let ll = S2.S2_latlng.of_point (S2.S2_cap.center u) in
      check_bool
        "intersects"
        ~expected:(bool_of_json_exn (member "intersects" c))
        ~actual:(S2.S2_cap.intersects e0 a0);
      check_float
        ~eps:k_degree_eps
        "lat_deg"
        ~expected:(float_of_json_exn (member "lat_deg" c))
        ~actual:(Float_u.to_float (S2.S1_angle.degrees (S2.S2_latlng.lat ll)));
      check_float
        ~eps:k_degree_eps
        "lng_deg"
        ~expected:(float_of_json_exn (member "lng_deg" c))
        ~actual:(Float_u.to_float (S2.S1_angle.degrees (S2.S2_latlng.lng ll)));
      check_float
        ~eps:k_degree_eps
        "radius_deg"
        ~expected:(float_of_json_exn (member "radius_deg" c))
        ~actual:(Float_u.to_float (S2.S1_angle.degrees (S2.S2_cap.radius_angle u)))
    | "two_large" ->
      check_bool
        "is_full"
        ~expected:(bool_of_json_exn (member "is_full" c))
        ~actual:(S2.S2_cap.is_full (S2.S2_cap.union f0 g0))
    | "hemi_union_complement" ->
      check_bool
        "is_full"
        ~expected:(bool_of_json_exn (member "is_full" c))
        ~actual:(S2.S2_cap.is_full (S2.S2_cap.union hemi (S2.S2_cap.complement hemi)))
    | _ -> Alcotest.fail ("unknown union case " ^ name))
;;

let test_encode_decode f () =
  let j = member "encode_decode" f in
  let cap = cap_of_json (member "roundtrip" j) in
  let encoded = S2.S2_cap.encode cap in
  let dec = S2.S2_cap.Option.value_exn (S2.S2_cap.decode encoded) in
  Alcotest.(check bool) "decode roundtrip" true (S2.S2_cap.equal dec cap);
  let floats = to_list (member "encoded" j) in
  List.iteri floats ~f:(fun i elt ->
    let expected = float_of_json_exn elt in
    let off = i * 8 in
    let actual = get_le_f64_from_string encoded off in
    check_float (sprintf "encode[%d]" i) ~expected ~actual)
;;

let () =
  let fixture = load_fixture "s2cap.json" in
  Alcotest.run
    "S2_cap"
    [ "basic", [ Alcotest.test_case "Basic" `Quick (test_basic fixture) ]
    ; "add_cap", [ Alcotest.test_case "AddCap" `Quick (test_add_cap fixture) ]
    ; "rect_bound", [ Alcotest.test_case "RectBound" `Quick (test_rect_bound fixture) ]
    ; "expanded", [ Alcotest.test_case "Expanded" `Quick (test_expanded fixture) ]
    ; "centroid", [ Alcotest.test_case "Centroid" `Quick (test_centroid fixture) ]
    ; "union", [ Alcotest.test_case "Union" `Quick (test_union fixture) ]
    ; ( "encode_decode"
      , [ Alcotest.test_case "EncodeDecode" `Quick (test_encode_decode fixture) ] )
    ; ( "quickcheck"
      , [ Alcotest.test_case "contains_center" `Quick quickcheck_contains_center
        ; Alcotest.test_case
            "complement_complement"
            `Quick
            quickcheck_complement_complement
        ; Alcotest.test_case "union_contains_both" `Quick quickcheck_union_contains_both
        ; Alcotest.test_case "add_point_contains" `Quick quickcheck_add_point_contains
        ; Alcotest.test_case "empty_subset_any" `Quick quickcheck_empty_subset_any
        ; Alcotest.test_case "any_subset_full" `Quick quickcheck_any_subset_full
        ] )
    ]
;;
