...
(* ---------- Quickcheck: edge-clipping invariants ------------------------- *)
...
let quickcheck_face_segments_in_uv_rect () =
...
let quickcheck_clip_to_padded_face_in_uv_rect () =
  Base_quickcheck.Test.run_exn
    (module S2_point_pair)
    ~config:qc_config
    ~f:(fun { S2_point_pair.a; b } ->
...
let quickcheck_clip_edge_inside_unit_rect () =
  Base_quickcheck.Test.run_exn
    (module R2_segment_in_unit_square)
    ~config:qc_config
    ~f:(fun { R2_segment_in_unit_square.a; b } ->
      (* A segment whose endpoints already lie in [-1,1]^2 should clip to
         itself (up to numerical noise). *)
...
let () =
  Alcotest.run
    ...
    ; ( "quickcheck"
      , [ Alcotest.test_case
            "face_segments_in_uv_rect"
            `Quick
            quickcheck_face_segments_in_uv_rect
        ; Alcotest.test_case
            "clip_to_padded_face_in_uv_rect"
            `Quick
            quickcheck_clip_to_padded_face_in_uv_rect
        ; Alcotest.test_case
            "clip_edge_inside_unit_rect"
            `Quick
            quickcheck_clip_edge_inside_unit_rect
        ] )
    ]
;;
