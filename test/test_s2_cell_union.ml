...
let quickcheck_union_is_superset () =
...
let quickcheck_intersection_self () =
  Base_quickcheck.Test.run_exn (module Cell_union_sample) ~config:qc_config ~f:(fun s ->
    let u = cu_of_sample s in
...
let quickcheck_difference_self () =
  Base_quickcheck.Test.run_exn (module Cell_union_sample) ~config:qc_config ~f:(fun s ->
    let u = cu_of_sample s in
...
let quickcheck_union_contains_intersection () =
  Base_quickcheck.Test.run_exn
    (module Cell_union_pair)
    ~config:qc_config
    ~f:(fun { Cell_union_pair.a; b } ->
...
let quickcheck_intersects_iff_nonempty_intersection () =
  Base_quickcheck.Test.run_exn
    (module Cell_union_pair)
    ~config:qc_config
    ~f:(fun { Cell_union_pair.a; b } ->
...
let () =
  run
    ...
    ; ( "quickcheck"
      , [ test_case "normalize_idempotent" `Quick quickcheck_normalize_idempotent
        ; test_case "union_is_superset" `Quick quickcheck_union_is_superset
        ; test_case "union_self" `Quick quickcheck_union_self
        ; test_case "intersection_self" `Quick quickcheck_intersection_self
        ; test_case "difference_self" `Quick quickcheck_difference_self
        ; test_case
            "union_contains_intersection"
            `Quick
            quickcheck_union_contains_intersection
        ; test_case
            "intersects_iff_nonempty_intersection"
            `Quick
            quickcheck_intersects_iff_nonempty_intersection
        ] )
    ]
;;
